...
### Configuración en el Dashboard de Cloudinary:
1. **Preajuste de carga (Upload Preset):** Creado como `sigi_preset` con **Modo de firma = No firmado (Unsigned)**.
2. **Carpeta de almacenamiento:** Las fotos se agrupan automáticamente bajo la carpeta `sigi_incidentes/`.
...
* **Parámetros del Formulario:**
  * `tipo`: ID entero del tipo de incidente.
  * `fecha`, `hora`: Datos temporales.
  * `lat`, `lng`: Coordenadas geográficas WGS84 (PostGIS).
  * `descripcion`, `direccion`: Texto del reporte.
  * `foto` *(Opcional)*: Archivo binario de imagen (PNG, JPG, JPEG, WEBP).
